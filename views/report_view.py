class ReportView:
    ...
    def show_players_in_tournament(tournament):
        ...
        sorted_players = sorted(
            tournament.players,
            key=lambda player_: player_.first_name + player_.last_name,
        )
    ...
    def show_rounds_and_matches(tournament):
        ...
        for round_instance in tournament.rounds:
            ...
            for match in round_instance.matches:
                print(
                    f"Match: {match.match_result[0][0].first_name}"
                    f" {match.match_result[0][0].last_name}"
                    f" score {match.match_result[0][1]}"
                    f" - {match.match_result[1][0].first_name} "
                    f"{match.match_result[1][0].last_name}"
                    f" score {match.match_result[1][1]}")
